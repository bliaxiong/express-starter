...
const getEmailFromGithubProfile = (profile) => {
  let email;
...
const addAvatarToProfile = (provider, url, profile) => {
  if (!profile.avatars)
    profile.avatars = {};
...
repo.assignResetPswToken = (email, token) => {
...
repo.changeProfileData = (userId, reqBody) => {
...
repo.removeUserById = (userId) => {
  return db.User.destroy({ where: { id: userId } });
};

repo.changeUserPassword = (userId, newPassword) => {
  return db.User.findById(userId)
    .then((user) => {
      if (!user)
...
repo.unlinkProviderFromAccount = (provider, userId) => {
  return db.User.findById(userId)
    .then((user) => {
      if (!user)
        throw 'User was not found.';
 ...
 * Facebook
 */
repo.linkFacebookProfile = (userId, accessToken, refreshToken, profile) => {
...
repo.createAccFromFacebook = (accessToken, refreshToken, profile) => {
  if (!profile._json) {
    ...
    .then((existingUser) => {
      ...
      return db.User.findOne({ where: { email: profile._json.email } })
        .then((emailUser) => {
          ...
          let user = db.User.build({ facebookId: profileId });
 ...
 * GitHub
 */
repo.linkGithubProfile = (userId, accessToken, tokenSecret, profile) => {
...
repo.createAccFromGithub = (accessToken, tokenSecret, profile) => {
  const profileId = profile.id.toString();
    ...
    .then((existingUser) => {
      ...
      return db.User.findOne({ where: { email: email } })
        .then((emailUser) => {
          ...
          let user = db.User.build({ githubId: profileId });
          user.email = email;
          user.tokens = { github: accessToken };
          user.profile = {
            name: profile.displayName,
 ...
 * Twitter
 */
repo.linkTwitterProfile = (userId, accessToken, tokenSecret, profile) => {
...
repo.createAccFromTwitter = (accessToken, tokenSecret, profile) => {
  return db.User.findOne({ where: { twitterId: profile.id.toString() } })
    .then((existingUser) => {
      if (existingUser)
        return existingUser;

      let user = db.User.build({ twitterId: profile.id.toString() });
 ...
 * Google
 */
repo.linkGoogleProfile = (userId, accessToken, tokenSecret, profile) => {
...
repo.createAccFromGoogle = (accessToken, tokenSecret, profile) => {
  return db.User.findOne({ where: { googleId: profile.id.toString() } })
    .then((existingUser) => {
      if (existingUser)
        return existingUser;
      ...
      return db.User.findOne({ where: { email: profile.emails[0].value } })
        .then((existingEmailUser) => {
          ...
          let user = db.User.build({ googleId: profile.id.toString() });
 ...
 * LinkedIn
 */
repo.linkLinkedInProfile = (userId, accessToken, tokenSecret, profile) => {
...
repo.createAccFromLinkedIn = (accessToken, tokenSecret, profile) => {
  return db.User.findOne({ where: { linkedInId: profile.id.toString() } })
    .then((existingUser) => {
      if (existingUser)
        return existingUser;
      ...
      return db.User.findOne({ where: { email: profile._json.emailAddress } })
        .then((existingEmailUser) => {
          ...
          let user = db.User.build({ linkedInId: profile.id.toString() });
